...
function lunch()
{
    ...
    if [ "$1" ] ; then
        ...
        print_lunch_menu
        echo -n "Which would you like? [disco_hammerhead-userdebug] "
        read answer
    fi
    ...
    if [ -z "$answer" ]
    then
        selection=disco_hammerhead-userdebug
    ...
    then
      if [ $exit_status -ne 0 ]
      then
        # if we can't find a product, try to grab it off the ProjectDisco github
        T=$(gettop)
        pushd $T > /dev/null
